...
namespace Unity.FPS.Game
{
    ...
    public class WeaponController : MonoBehaviour
    {
        ...
        [Tooltip("Allows ammo to recover automatically over time")]
        public bool PassiveAmmoRefill = true;

        [Tooltip("Uses reserve ammo plus a magazine instead of a single pooled ammo value")]
        ...
        void Reload()
        {
            if (UseMagazineSystem)
            {
                int missingAmmo = CurrentClipSize - Mathf.FloorToInt(m_CurrentAmmo);
                int ammoToLoad = Mathf.Min(m_CarriedPhysicalBullets, missingAmmo);
                m_CurrentAmmo = Mathf.Clamp(m_CurrentAmmo + ammoToLoad, 0, CurrentClipSize);
                m_CarriedPhysicalBullets = Mathf.Max(0, m_CarriedPhysicalBullets - ammoToLoad);
            }
        ...
        public void StartReloadAnimation()
        {
            if (UseMagazineSystem)
            {
                if (m_CurrentAmmo >= CurrentClipSize || m_CarriedPhysicalBullets <= 0)
                {
                    return;
                }
                ...
                if (WeaponAnimator != null)
                {
                    WeaponAnimator.speed = ReloadSpeedMultiplier;
                    WeaponAnimator.SetTrigger("Reload");
                }
                else if (TryInstantReload())
                {
                    return;
                }
        ...
        void UpdateAmmo()
        {
            if (UseMagazineSystem &&
                ...
                !IsReloading &&
                !IsCharging &&
                Mathf.FloorToInt(m_CurrentAmmo) <= 0 &&
                m_CarriedPhysicalBullets > 0)
            {
                StartReloadAnimation();
            }

            if (PassiveAmmoRefill &&
                AutomaticReload &&
            ...
            if (UseMagazineSystem)
            {
                CurrentAmmoRatio = m_CurrentAmmo / CurrentClipSize;
            }
            else if (MaxAmmo == Mathf.Infinity)
            {
                CurrentAmmoRatio = 1f;
            }
            else
        ...
        {
            float ammoCap = UseMagazineSystem ? CurrentClipSize : MaxAmmo;
            m_CurrentAmmo = Mathf.Clamp(m_CurrentAmmo - amount, 0f, ammoCap);
        ...
        public void AddAmmo(float amount)
        {
            ...
            if (UseMagazineSystem)
            {
                m_CarriedPhysicalBullets = Mathf.Clamp(m_CarriedPhysicalBullets + Mathf.RoundToInt(amount), 0, MaxAmmo);
                return;
            }
        ...
        {
            DamageMultiplier = Mathf.Max(1f, DamageMultiplier + amount);
        }

        public void AddReloadSpeedMultiplier(float amount)
        {
            ReloadSpeedMultiplier = Mathf.Max(1f, ReloadSpeedMultiplier + amount);
        }

        public void AddClipSizeBonus(int amount)
        {
            m_AdditionalClipSize = Mathf.Max(0, m_AdditionalClipSize + amount);
            m_CurrentAmmo = Mathf.Clamp(m_CurrentAmmo, 0, CurrentClipSize);
        }

        public void AddProjectileRangeMultiplier(float amount)
        {
            ProjectileRangeMultiplier = Mathf.Max(1f, ProjectileRangeMultiplier + amount);
        }
        ...
        public void SetMagazineDefaults(int clipSize, int reserveAmmo)
        {
            ...
            MaxAmmo = Mathf.Max(0, reserveAmmo);
            m_CurrentAmmo = CurrentClipSize;
            m_CarriedPhysicalBullets = MaxAmmo;
            CurrentAmmoRatio = m_CurrentAmmo / CurrentClipSize;
        }
        ...
        public void RefillMagazineFromReserve()
        {
            ...
            int missingAmmo = CurrentClipSize - Mathf.FloorToInt(m_CurrentAmmo);
            if (missingAmmo <= 0 || m_CarriedPhysicalBullets <= 0)
            {
                return;
            }
        ...
        {
            if (!UseMagazineSystem || m_CarriedPhysicalBullets <= 0)
            {
                return false;
            }
